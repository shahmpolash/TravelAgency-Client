...
const AddProductVariation = () => {
  ...
  useEffect(() => {
    fetch(`https://travelagency-a887203e1d88.herokuapp.com/product-variation/${id}`)
      .then((res) => res.json())
  ...
  const handleVariation = (event) => {
    ...
    const url = `https://travelagency-a887203e1d88.herokuapp.com/add-variation/${variation._id}`;
    fetch(url, {
      method: 'PUT',
      headers: {
